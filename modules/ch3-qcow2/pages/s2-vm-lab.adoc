...
+
It installs the `cloud-init` and `qemy-guest-agent` packages and enable their respective Systemd units, over the `httpd-bootc` bootc container image.
...
.. Verify that your _test VM_ has an active cloud-init and guest agent services, and terminate its SSH session.
+
[source,subs="verbatim,quotes"]
--
$ *systemctl is-active cloud-init*
