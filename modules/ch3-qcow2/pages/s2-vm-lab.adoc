...
Make sure you verified your test environment by following the xref:ch1-bib:s3-prereqs-lab.doc[first lab] of this course.

== Instructions

You will use the bootc image builder utility to create a custom Qcow2 image, which day-1 configurations from both bootc image builder and Cloud-init, and provision a new VM from it.
You will find that the configurations from Cloud-init were NOT applied, and that libvirt cannot introspect the VM instance.
To solve these issues, you will add Cloud-init and the QEMU guest agent to the bootc container image, build a new Qcow2 image, and validate the fix by provisioning another VM.

include::ch1-bib:partial$set-permissive.adoc[]
...
If you do not, please perform the xref:ch1-bib:s3-prereqs-lab.doc[first lab], which builds and publishes that image.
+
[source,subs="verbatim,quotes"]
--
...
If you do not, please perform the preparation tasks from the xref:ch1-bib:s3-prereqs-lab.doc[first lab].
+
[source,subs="verbatim,quotes"]
--
$ *virsh list --all*
...
.. Review the example bootc image builder TOML configuration file named `config.toml`.
+
It creates an initial user, so we can access VMs created from it and perform onboarding.
+
[source,subs="verbatim,quotes"]
--
...
You cannot perform them with bootc image builder.
You must either use another day-1 customization technology, or have the network settings hard-coded in your bootc container image.
...
3. Create a custom VM image in Qcow2 format.
...
That file would be distributed to personnel responsible for installing actual edge devices.
+
Make sure the file name matches the one expected by the `virt-install.sh` script.
+
[source,subs="verbatim,quotes"]
--
$ *cp output/qcow2/disk.qcow2 httpd-system.qcow2*
--
...
4. Create a local VM, using the Qcow2 image.
...
--
+
WARNING: The `--remove-all-storage` deletes the Qcow2 image which boots the VM, so it is important to NOT use the copy in the `output` directory.

5. Verify that the test VM runs a web server with the mock web application files, but was NOT customized by Cloud-init, and it is NOT running a guest agent either.
...
.. Verify that the Apache Web Server is active is your _test VM_ and it contains the files of the mock web application.
+
[source,subs="verbatim,quotes"]
--
...
.. Verify that your _test VM_ does NOT have a QEMU guest agent either, and terminate its SSH session.
+
[source,subs="verbatim,quotes"]
--
...
.. Verify that the host can access the mock web application, through  port-forwarding, similar to how SSH access works.
+
[source,subs="verbatim,quotes"]
--
$ *curl 127.0.0.1:8080*
...
.. Verify that your local hypervisor cannot get information about the operating system inside your _test VM_.
+
[source,subs="verbatim,quotes"]
--
$ *virsh guestinfo bootc-qcow2*
error: Guest agent is not responding: QEMU guest agent is not connected
--

6. Stop your _test VM_, so you can create a new one with supports Cloud-init and the QEMU guest agent.
+
[source,subs="verbatim,quotes"]
--
$ *virsh destroy bootc-qcow2*
...
7. Create a bootc container image with Cloud-init and guest agent services.
...
+
It installs the `cloud-init` and `qemu-guest-agent` packages and enable their respective Systemd units, over the `httpd-system` bootc container image.
...
--
+
NOTE: You should see the hostname change in the VM's login prompt, as a sign that, this time, it got customizations from Cloud-init.

9. Access your new _test VM_ using the initial user created by Cloud-init.

.. Remove the previous host key from your known hosts file and open an SSH connection, but using the `cloud-user` user instead of the `core` user.
+
[source,subs="verbatim,quotes"]
--
$ *ssh-keygen -R '[127.0.0.1]:8022'*
...
.. Verify that your _test VM_ has active Cloud-init and guest agent services, and terminate its SSH session.
+
[source,subs="verbatim,quotes"]
--
$ *systemctl is-active cloud-init*
...
.. Verify that the mock web application is available.
+
[source,subs="verbatim,quotes"]
--
$ *curl 127.0.0.1:8080*
...
.. Verify that your hypervisor can now get information from the operating system inside your _test VM_.
+
[source,subs="verbatim,quotes"]
--
$ *virsh guestinfo bootc-cloud-init*
user.count          : 0
os.id               : rhel
...
The strategy demonstrated here, of layering bootc container images to add services and settings specific to a cloud provider over a more generic bootc container image, is just a suggestion.
