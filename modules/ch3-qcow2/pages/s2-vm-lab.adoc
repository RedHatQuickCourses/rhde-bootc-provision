...
.. [ Can I set the hostname on the config.toml to avoid seeing "workstation" as the hostname of the test VM?]

6. Create a bootc container image with cloud-init services.

.. https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/10/html/configuring_and_managing_cloud-init_for_rhel/creating-a-virtual-machine-with-cloud-init

.. Looks very convoluted, creating multiple files and bundling an ISO... it seems that virt-install can take the files directly, and create the ISO on the fly, which is easier.

.. Got the cloud-init package (from my image) but not the hostname :-( The cloud-user user works with password and key... guess it's overriding the hostname? No, it was a typo in my command :-)
