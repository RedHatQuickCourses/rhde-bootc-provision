...
Make sure you verified your test environment by following the xref:ch1-bib:s3-prereqs-lab.adoc[first lab] of this course.
...
+
--
... Locale, keyboard, and other system settings so the installation can proceed unattended.
+
... A new partition table, with default automatic partitioning for RHEL.
+
... Dynamic network configuration, using DHCP.
+
...
+
... Bootc image builder creates an ISO image and uses the TOML file to pick its kickstart script.
+
...
.. Verify that there are two kickstart scripts inside the ISO image.
+
[source,subs="verbatim,quotes"]
--
$ **ls temp-iso/*ks**
...
.. Verify that the included `osbuild-base.ks` file contains an `ostreecontainer` command.
It also contains a `%post` script that changes the bootc container image reference from the installation media to the reference given to bootc image builder. 
+
[source,subs="verbatim,quotes"]
--
...
.. Unmount the generated ISO image.
+
[source,subs="verbatim,quotes"]
--
...
6. Install RHEL in a local VM, using the custom installation ISO, without accessing the container registry.
...
.. Verify that the Apache Web Server is active is your _test VM_.
+
[source,subs="verbatim,quotes"]
--
