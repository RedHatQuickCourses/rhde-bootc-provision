...
+
... Bootc image builder uses the `httpd-bootc:v1.0` container image, which was previously pulled.
--
+
[source,subs="verbatim,quotes"]
--
...
--
$ *mkdir temp-iso*
$ *sudo mount -o loop,ro httpd-bootc.iso temp-iso*
--
...
%post --erroronfail
bootc switch --mutate-in-place --transport registry registry.lab.example.com:5000/httpd-bootc:v1.0
--
...
[source,subs="verbatim,quotes"]
--
$ *iso-info httpd-bootc.iso | grep Volume*
...
$ *podman search registry.lab.example.com:5000/*
NAME                                                                  DESCRIPTION
registry.lab.example.com:5000/httpd-bootc
...
--
