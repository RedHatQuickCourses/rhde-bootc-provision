...
Notice that the kickstart script does NOT include an `ostreecontainer` command.
+
[source,subs="verbatim,quotes"]
--
...
--
$ *mkdir temp-iso*
$ *sudo mount -o loopack,ro httpd-bootc.iso tempiso*
--
...
[source,subs="verbatim,quotes"]
--
$ *head -n 5 temp-iso/osbuild-base.ks*
GRAB OUTPUT
--
...
.. Grab the disk label of the generated ISO image.
Notice it matches the kernel arguments on the `virt-install.sh` script.
+
[source,subs="verbatim,quotes"]
--
$ *????*
...
.. Open an SSH session to the `utility` VM.
...
.. Close your SSH session and verify that your _development machine_ cannot access the private registry anymore.
+
[source,subs="verbatim,quotes"]
--
$ *exit*
...
6. Install RHEL in a VM using the custom installation ISO.

.. To demonstrate that the installation works in a disconnected Open an SSH session to the `utility` VM and 
...
.. Verify that the system is installed Run bootc status

7. Verify that the test VM runs a web server with the mock web applicaiton files.
